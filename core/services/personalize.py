...
def default_body(quiz: dict[str, Any], influences: list[dict[str, Any]]) -> str:
    ...
    if inf_text:
        ...
        return f"{short}. В запросе «{intent_label}» важно сначала назвать, что уже не даёт опоры в теме «{focus_label}»."[:320]
    return (
        f"Сейчас особенно заметно, где привычная роль стала тесной в теме «{focus_label}». "
        f"В запросе «{intent_label}» полезно честно увидеть, что больше не работает."
    )[:320]
